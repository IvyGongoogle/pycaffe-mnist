...
img = os.path.join(root, 'data/test', '29.png')    #随机找的一张待测图片
...
im=caffe.io.load_image(img, color=False)  # 加载图片.此处踩坑。color默认值为True，会把灰度图（单通道）存储为彩图（3通道），实际上mnist数据是单通道的。cv2.imread()默认也会将其转为3通道，很坑
